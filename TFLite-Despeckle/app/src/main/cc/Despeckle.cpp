...
Despeckle::Despeckle(const void* model_data, size_t model_size,
                             bool use_gpu) {
  // Load the model
...
bool Despeckle::IsInterpreterCreated() {
  if (!interpreter_) {
    return false;
  } else {
    return true;
...
std::unique_ptr<int[]> Despeckle::DoDespeckle(int* lr_img_rgb) {
  ...
  float output_buffer[kNumberOfInputPixels * kImageChannels];
  ...
  auto rgb_colors = std::make_unique<int[]>(kNumberOfInputPixels);
  for (int i = 0; i < kNumberOfInputPixels; i++) {
